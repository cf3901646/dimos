...
class DamiaoWholeBodyAdapter(ABC):
    ...
    @property
    def kinematic_model_path(self) -> Path | None:
        """Return the subclass's arm URDF without resolving it at import time."""
        return None
    ...
    def _load_kinematic_model(self) -> None:
        if not self.kinematic_joint_names:
            return
        model_path = self.kinematic_model_path
        if model_path is None or not model_path.is_file():
            raise ValueError("Damiao arm safety requires an existing kinematic URDF")
