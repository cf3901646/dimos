...
class G1DualArmIKTask(BaseControlTask):
    ...
    ``frame_id = f"{task_name}/left"`` / ``.../right"``; the card binds
    ``coordinator_cartesian_command`` ungated (``direct``), so
    ``on_cartesian_command`` owns the addressing — it drops foreign task
    names and reads the hand off the suffix.
    """
    ...
    def on_cartesian_command(self, pose: Pose | PoseStamped, t_now: float) -> bool:
        ...
        The frame_id is "<task>/left" or "<task>/right". The port is
        delivered ungated, so commands addressed to another task are
        silently not ours; a warning is only worth raising when the task
        name matches but the hand suffix is malformed.
        """
        frame_id = getattr(pose, "frame_id", "")
        task_name, _, hand = frame_id.rpartition("/")
        if task_name != self._name:
            return False
    ...
    def set_estop(self, estopped: bool) -> None:
        """Latch/clear E-STOP. On latch, disengage and drop targets and the
        held solution so the task goes inert (is_active() False) instead of
        keeping teleop-driven arms live."""
        with self._lock:
